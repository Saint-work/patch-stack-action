...
For fork-local commits that you want to preserve on `main`, use the `fork:` prefix. The current sync logic preserves only commits whose subject starts with `fork: `. Do **not** use the reserved `patch-stack: ` prefix unless the commit was generated by the workflow itself. If a human-authored commit on `main` uses `patch-stack: `, the sync logic may treat it as disposable generated output instead of preserved fork plumbing, and if it uses some other prefix (`fix:`, `ci:`, etc.) it will currently be skipped instead of preserved.
